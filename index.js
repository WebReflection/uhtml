self.uhtml = (function (exports) {
  ...
  const svg$1 = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
  const range = document.createRange();
  const createSVG = ml => {
    svg$1.innerHTML = ml;
    range.setStartBefore(svg$1.firstChild);
    range.setEndAfter(svg$1.lastChild);
    return range.extractContents();
  };

  const create$1 = (ml, type) => (type === 'svg' ? createSVG : createHTML)(ml);
  ...
  const mapTemplate = (type, template) => {
    ...
    const content = create$1(text, type);
